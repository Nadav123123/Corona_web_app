...
def date_range(all_df):
    ...
    range_ = pd.date_range(min_date, max_date, freq= '7d')
    max_date = range_[-1]
...
def another_fig(user_input, df, geo, user_date, per_mil, days_offset = 0):
    ofsset = False
    range_ = pd.date_range(user_date, user_date + dt.timedelta(days=6), freq='d')
...
def app(df_main):
    ...
    # function calling
    all_df, counties = set_data(df_main)
    ...
    date_to_filter = pd.to_datetime(st.slider('Date Slider', min_date, max_date, step = dt.timedelta(days = 7)
                                    , value=max_date))
    ...
    if len_lis > 0:
        st.write(f'**map date range: ({range_[0].date()}) to ({range_[-1].date()})'
                f', the data shown is an average for the week')
        st.write(fig_lis[0])
    if len(fig_lis) > 1:
        range_ = new_range
        for i in fig_lis[1:]:
            st.write(
                f'**map date range: ({range_[0].date()}) to ({range_[-1].date()}), the data shown is an average for the week')
            st.write(i)
